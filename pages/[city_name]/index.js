...
export default function MyListings(props) {
  ...
  function hellochange(vall) {
    if (
      ...
      return (
        <h1 className="main-title mb-0 mt-2 mb-md-0">
          Top {vall} New Construction Detached, Semi-detached, Townhomes &
          Condos in {capitalizeFirstLetter(routee.query.city_name)}
        </h1>
      );
    }
  }
  function hellochange2(vall) {
    if (
      ...
      return (
        <h2 className="main-subtitle d-flex justify-content-between mt-3 mt-md-2">
          {vall} New upcoming, selling and sold new construction homes in{" "}
          {capitalizeFirstLetter(routee.query.city_name)} | Check out plans,
          pricing and availability on Homebaba
        </h2>
      );
    }
  }
...
export async function getStaticProps({ params }) {
  ...
  const posts = await axios.get(
    `https://api.homebaba.ca/api/pre-constructions-city/${params.city_name}/?page=${page}`
  );
  return {
    props: {
